...
export default function Header() {
  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[52rem] sm:rounded-full dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75"
      ...
      <nav className="hidden sm:flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-full max-w-[48rem] flex-wrap items-center justify-between px-4 gap-y-1 text-[0.85rem] font-medium text-gray-500 sm:flex-nowrap sm:gap-3">
          <div className="flex items-center justify-center flex-wrap sm:flex-nowrap">
            {links.map((link) => (
              ...
              >
                <Link
                  className={clsx(
                    "flex w-full items-center justify-center px-1.5 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300 sm:px-2.5 whitespace-nowrap",
                    {
